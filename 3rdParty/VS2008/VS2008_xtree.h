...
#pragma once
#ifndef _VS2008_XTREE_
#define _VS2008_XTREE_
#ifndef RC_INVOKED
#include <functional>
...
protected:
	...
	{	// add node with value next to _Wherenode, to left if _Addnode
		if (max_size() - 1 <= _Mysize)
			throw std::length_error("map/set<T> too long"); //_THROW(std::length_error, "map/set<T> too long");
	...
	{	// report an out_of_range error
		throw std::out_of_range("invalid map/set<T> iterator"); //_THROW(std::out_of_range, "invalid map/set<T> iterator");
	}

	static void _Xinvarg()
	{	// report an invalid_argument error
		throw std::invalid_argument("invalid map/set<T> argument"); //_THROW(std::invalid_argument, "invalid map/set<T> argument");
	}
